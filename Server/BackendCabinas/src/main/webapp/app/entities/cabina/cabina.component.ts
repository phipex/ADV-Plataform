...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/cabina'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Cabina) {
        return item.id;
    }
    registerChangeInCabinas() {
        this.eventSubscriber = this.eventManager.subscribe('cabinaListModification', (response) => this.loadAll());
    }
